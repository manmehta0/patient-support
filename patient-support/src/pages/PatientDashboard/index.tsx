...
// Define the types for the expected structure of the data
interface DashboardData {
  patient_info: any;
  outcome_prediction: any;
  clinical_trials: any;
  treatment_recommendations: any;
  wearable_data_analysis: any;
  recent_feedback: any;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <ClinicalTrials trials={dashboardData.clinical_trials} getStudy={() => true} />
        ...
        <FeedbackAnalysis
          feedback={dashboardData.recent_feedback}
          sentiment={dashboardData.sentiment_analysis}
        />
      </div>
      <button
        onClick={() => setIsChatbotOpen(!isChatbotOpen)}
        className="fixed bottom-4 right-4 p-3 bg-purple-600 text-white rounded-full shadow-lg hover:bg-purple-700 focus:outline-none flex items-center justify-center"
      >
      ...
      {/* Chatbot Flyout */}
      <div
        className={`fixed bottom-3 right-0 h-[95%] transition-transform duration-300 ease-in-out transform ${isChatbotOpen ? 'translate-x-0' : 'translate-x-full'} bg-gray-800 border border-gray-700 rounded-lg shadow-lg`}
      >
